...
test('mobile viewport - responsive layout', async ({ page }) => {
  test.skip(!page.viewportSize()?.width || page.viewportSize()?.width > 600, 'Desktop viewport detected');
...
test('mobile viewport - touch interactions', async ({ page }) => {
  ...
  await page.locator('.cell').nth(0).click(); // Using click instead of tap for compatibility
  await expect(page.locator('.cell').nth(0)).toHaveText('X');
